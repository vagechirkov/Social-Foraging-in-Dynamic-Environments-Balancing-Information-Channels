...
belief_selectivity_array=(6.0 9.0 12.0)
...
process_noise_scale_array=(0.05 0.1 0.01 0.2)
                                ...
                                for t_speed in "${target_speed_array[@]}"; do
                                    for c_priv in "${cost_priv[@]}"; do
                                        for c_bel in "${cost_belief[@]}"; do
                                            sbatch 3_channel_cpu_hpc_itb.sh "$t_speed" "$c_priv" "$c_bel" "$dim" "$n_agents" "$gamma_belief" "$belief_selectivity" "$base_noise" "$dist_noise_scale_priv" "$target_persistence" "$process_noise_scale"
                                        done
                                    done
                                done
                            done
